...
class PdfController extends Controller
{
    ...
    public function attendance_pdf($id, $date_from = null, $date_to = null)
    {
        ...
        if (! is_null($date_from) && ! is_null($date_from)) {
            $data['absent']
                = attendance::where('department_id', $data['department']->id)->whereBetween('attendance_date', [$date_from, $date_to])->where('status', 2)->count();
            $data['came']
                = attendance::where('department_id', $data['department']->id)->whereBetween('attendance_date', [$date_from, $date_to])->where('status', 1)->count();
            $data['e3tezar'] = attendance::where('department_id', $data['department']->id)->whereBetween('attendance_date', [$date_from, $date_to])->where('status', 3)->count();
            $data['attendance'] = attendance::with('users')->where('department_id', $data['department']->id)->whereBetween('attendance_date', [$date_from, $date_to])->orderby('attendance_date', 'asc')->get();
            $data['dates'] = attendance::select('attendance_date')->whereBetween('attendance_date', [$date_from, $date_to])->distinct()->get();
        } else {
            $data['came'] = attendance::where('department_id', $data['department']->id)->where('status', '1')->count();
            $data['absent'] = attendance::where('department_id', $data['department']->id)->where('status', '2')->count();
            $data['e3tezar'] = attendance::where('department_id', $data['department']->id)->where('status', '3')->count();
            $data['attendance'] = attendance::with('users')->where('department_id', $data['department']->id)->orderby('attendance_date', 'asc')->get();
